 ...
 * - Separate declarations and definitions to reduce header coupling and improve build hygiene.
 * - Add explicit bounds checking for all array accesses to prevent undefined behavior on malformed input.
 * - Replace variable-length arrays with dynamic allocations or fixed maximum sizes to clarify ownership and lifetime.
 * - Return error codes instead of printing directly in helpers so callers can respond appropriately.
 * - Normalize tab/space handling and expose configuration through function parameters where sensible.
 * - Add unit tests covering nested comments, escaped delimiters, and empty input to ensure edge cases are handled.
 */
 ...
 * @return Index of the next matching character or 0 if not found before the limit.
 */
 ...
 * @return 1 when the preceding character matches, 0 when it does not, and -1 on out-of-range input.
 */
 ...
 * @param c   Character to test for.
 * @param lim Maximum valid length of the buffer.
 * @return 1 when the following character matches, 0 when it does not, and -1 on out-of-range input.
 */
